...
/**
 * Translate the netcat form of ProxyCommand into an equivalent ProxyJump hop.
 *
 * `ProxyCommand ssh bastion -W %h:%p` is what ssh itself turns `ProxyJump
 * bastion` into, and configs generated by hand or by inventory tools carry the
 * long form far more often than the short one. Any other command is left alone:
 * a real subprocess is not something this server runs.
 */
function proxyJumpFromProxyCommand(command: string): string | null {
  ...
  for (let index = 1; index < tokens.length; index++) {
    ...
    if (host) {
      // A second operand means the command runs something on the hop.
      return null;
    }
  ...
  if (!netcatMode || !host || host.includes('%')) {
    return null;
  }
...
function matchHost(hostAlias: string, blocks: HostBlock[]): SshConfigEntry | null {
  ...
  for (const block of blocks) {
    ...
    if (!result.proxyJump && block.config.has('proxycommand')) {
      const hop = proxyJumpFromProxyCommand(block.config.get('proxycommand')!);
      if (hop) {
        result.proxyJump = hop;
      }
    }
  ...
  if (result.proxyJump?.includes('%r')) {
    // `-l %r` on the hop means "the user of the target", which is only known
    // once every matching block has been folded in.
    result.proxyJump = result.user
      ? result.proxyJump.replace(/%r/g, result.user)
      : result.proxyJump.replace(/(^|,)[^,@]*%r[^,@]*@/g, '$1');
  }
